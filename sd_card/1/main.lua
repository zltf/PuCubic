...
function loop()
    ...
    if rand > 0.9 then
        ...
        big_snow_list[idx] = snow
        idx = idx + 1
    end

    arduino.tft_img(70, 70, 100, 120, PP)
    arduino.tft_img(10, 170, 60, 50, MC)

    for idx, snow in pairs(big_snow_list) do
        arduino.tft_draw_circle(math.ceil(snow.x), math.ceil(snow.y), snow.r, 0)
        snow.x = snow.x - snow.s / 10 * math.sin(math.rad(ax))
        snow.y = snow.y + snow.s / 10 * math.cos(math.rad(ax))
        arduino.tft_draw_circle(math.ceil(snow.x), math.ceil(snow.y), snow.r, 0xFFFF)
        if snow.y > 255 then
            big_snow_list[idx] = nil
        end
    end
